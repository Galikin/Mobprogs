...
/*
 * $Log: unified_prog.c,v $
 * Unified MOB/OBJ/ROOM program system supporting both inline and vnum-driven progs
 *
 */
...
/* ---------------------------------------------------------------------
 * CMD_EVAL functions for each prog type
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	//case CHK_CLAN:
	  //  return( lval_char != NULL && lval_char->pcdata->clan == clan_lookup( buf ) );
	case CHK_RACE:
                       ...
                       CHAR_DATA *ch, const void *arg1, const void *arg2, int type )
{
    ...
    else if ( obj )
    {
        for ( prg = obj->pIndexData->oprogs; prg != NULL; prg = prg->next )
        {
            ...
            {
                program_flow( prg->vnum, prg->code, NULL, obj, NULL, ch, arg1, arg2 );
                return TRUE;   // keep stock behavior for objects
            }
        }
    }
    else if ( room )
    {
        ...
        for ( prg = room->rprogs; prg != NULL; prg = prg->next )
        {
            ...
            &&   number_percent() < prg->chance )
            {
                program_flow( prg->vnum, prg->code, NULL, NULL, room, ch, arg1, arg2 );
                fired = TRUE;
                // no return; keep looping for multiple progs
            }
        }
...
void p_greet_trigger( CHAR_DATA *ch, int type )
{
    ...
    else if ( type == PRG_OPROG )
    {
        for ( obj = ch->in_room->contents; obj != NULL; obj = obj->next_content )
        {
            if ( HAS_TRIGGER_OBJ( obj, TRIG_GRALL ) && !IS_SET(ch->act, PLR_WIZINVIS))
            {
    ...
    else if ( type == PRG_RPROG )
    {
        ...
        if ( HAS_TRIGGER_ROOM( room, TRIG_GRALL ) && !IS_SET(ch->act, PLR_WIZINVIS))
...
void p_rand_trigger( CHAR_DATA *mob )
{
    if ( HAS_TRIGGER_MOB( mob, TRIG_RANDOM ) )
	p_percent_trigger( mob, NULL, NULL, NULL, NULL, NULL, TRIG_RANDOM );
}

void p_entry_trigger( CHAR_DATA *ch )
{
    CHAR_DATA *mob;
    ...
    {
	if ( IS_NPC( mob )
	&&   HAS_TRIGGER_MOB( mob, TRIG_ENTRY ) )
	    p_percent_trigger( mob, NULL, NULL, ch, NULL, NULL, TRIG_ENTRY );
    }
    return;
}

void p_fight_trigger( CHAR_DATA *mob, CHAR_DATA *ch )
{
    if ( HAS_TRIGGER_MOB( mob, TRIG_FIGHT ) )
	p_percent_trigger( mob, NULL, NULL, ch, NULL, NULL, TRIG_FIGHT );
}


bool p_death_trigger( CHAR_DATA *killer, CHAR_DATA *mob )
{
    if ( IS_NPC(mob) && HAS_TRIGGER_MOB(mob, TRIG_DEATH) )
    {
        mob->position = POS_STANDING;
        p_percent_trigger(mob, NULL, NULL, killer, NULL, NULL, TRIG_DEATH);
        mob->position = POS_DEAD;
    }

    death_cry(mob);  /* Always do this like your old code */
    return FALSE;    /* No char_died check, just return FALSE */
}
 ...
 * ------------------------------------------------------------------------
 * WRAPPER FUNCTIONS
 * These maintain compatibility with existing inline mobprog system
 * They check for vnum-driven progs first, then fall back to inline progs
 * ------------------------------------------------------------------------
 */

/* 
 * Dual-path ACT trigger - checks both vnum-driven and inline progs
 */
void mprog_act_trigger( char *buf, CHAR_DATA *mob, CHAR_DATA *ch,
                               OBJ_DATA *obj, void *vo )
{
    /* First check vnum-driven mobprogs */
    if ( IS_NPC(mob) && mob->pIndexData->mprogs )
        p_act_trigger( buf, mob, NULL, NULL, ch, obj, vo, TRIG_ACT );
    
    /* Then check existing inline system - keep your existing function call here */
     mprog_act_trigger_inline( buf, mob, ch, obj, vo ); 
}

/*
 * Dual-path PERCENT trigger 
 */
bool mprog_percent_trigger( CHAR_DATA *mob, CHAR_DATA *ch, 
                                   OBJ_DATA *obj, void *vo, int type )
{
    ...
    /* First check vnum-driven mobprogs */
    if ( IS_NPC(mob) && mob->pIndexData->mprogs )
        triggered = p_percent_trigger( mob, NULL, NULL, ch, obj, vo, type );
    
    /* If not triggered, check existing inline system */
    if ( !triggered )
    {
        /* Call your existing mprog_percent_check function here */
         mprog_percent_check_inline( mob, ch, obj, vo, type );
    }
...
/*
 * Dual-path GREET trigger
 */
void mprog_greet_trigger( CHAR_DATA *ch )
{
    /* Check vnum-driven mobprogs first */
    p_greet_trigger( ch, PRG_MPROG );
    
    /* Then check existing inline system */
    mprog_greet_trigger_inline( ch );
}

/*
 * Dual-path SPEECH trigger
 */
void mprog_speech_trigger( char *txt, CHAR_DATA *mob )
{
    ...
    {
        if ( IS_NPC(vmob) && vmob != mob )
        {
            /* First check vnum-driven progs */
            if ( vmob->pIndexData->mprogs )
                p_act_trigger( txt, vmob, NULL, NULL, mob, NULL, NULL, TRIG_SPEECH );
...
/*
 * Dual-path GIVE trigger
 */
void mprog_give_trigger(CHAR_DATA *mob, CHAR_DATA *ch, OBJ_DATA *obj)
{
    /* First check vnum-driven mobprogs (modern triggers) */
    if (IS_NPC(mob) && mob->pIndexData->mprogs)
        p_give_trigger(mob, NULL, NULL, ch, obj, TRIG_GIVE);

    /* Then check legacy inline GIVE_PROG triggers */
    mprog_give_trigger_inline(mob, ch, obj);

    /* Then check legacy GIVE_WORD_PROG triggers */
    mprog_give_word_trigger(mob, ch, obj);
}


/*
 * Dual-path BRIBE trigger
 */
void mprog_bribe_trigger( CHAR_DATA *mob, CHAR_DATA *ch, int amount )
{
    /* First check vnum-driven mobprogs */
    if ( IS_NPC(mob) && mob->pIndexData->mprogs )
        p_bribe_trigger( mob, ch, amount );
    
    /* Then check existing inline system */
    mprog_bribe_trigger_inline( mob, ch, amount );
}

/*
 * Dual-path HITPRCNT trigger
 */
void mprog_hitprcnt_trigger( CHAR_DATA *mob, CHAR_DATA *ch )
{
    /* First check vnum-driven mobprogs */
    if ( IS_NPC(mob) && mob->pIndexData->mprogs )
        p_hprct_trigger( mob, ch );
    
    /* Then check existing inline system */
    mprog_hitprcnt_trigger_inline( mob, ch );
}

/*
 * Dual-path EXIT trigger
 */
bool mprog_exit_trigger(CHAR_DATA *ch, int dir)
{
    /* First check vnum-driven mobprogs */
    if (p_exit_trigger(ch, dir, PRG_MPROG))
        return TRUE;

    /* Then check existing inline system */
    return mprog_exit_trigger_inline(ch, dir);
}
...
void mprog_random_trigger( CHAR_DATA *mob )
{
	...
	p_rand_trigger( mob );
    mprog_random_trigger_inline( mob );
}
...
{
    if ( mob->pIndexData->mprogs )
	p_fight_trigger( mob, ch );
    mprog_fight_trigger_inline( mob, ch );
}
...
{
    if ( mob->pIndexData->mprogs )
	if ( p_death_trigger( killer, mob ) )
	    return TRUE;
    return mprog_death_trigger_inline( killer, mob );
    return FALSE;
}

void mprog_entry_trigger( CHAR_DATA *ch )
{
    p_entry_trigger( ch );
    mprog_entry_trigger_inline( ch );
}
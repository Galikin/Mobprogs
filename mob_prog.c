...
void mprog_percent_check_inline(CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj,
                         void *vo, int type)
{
    MPROG_DATA *mprg;
    char bugbuf[MAX_STRING_LENGTH];
 ...
 * trigger calls.
 */
void mprog_act_trigger_inline(char *buf, CHAR_DATA * mob, CHAR_DATA * ch,
                       OBJ_DATA * obj, void *vo)
{
...
void mprog_bribe_trigger_inline(CHAR_DATA * mob, CHAR_DATA * ch, int amount)
{
...
void mprog_death_trigger_inline(CHAR_DATA * mob, CHAR_DATA * actor)
{
    ...
    {
      mob->wait = 0;
      mprog_percent_check_inline(mob, actor, NULL, NULL, DEATH_PROG);
    }
...
void mprog_entry_trigger_inline(CHAR_DATA * mob)
{

    if (IS_NPC(mob)
        && (mob->pIndexData->progtypes & ENTRY_PROG))
        mprog_percent_check_inline(mob, NULL, NULL, NULL, ENTRY_PROG);

    return;

}

void mprog_fight_trigger_inline(CHAR_DATA * mob, CHAR_DATA * ch)
{

    if (IS_NPC(mob)
        && (mob->pIndexData->progtypes & FIGHT_PROG))
        mprog_percent_check_inline(mob, ch, NULL, NULL, FIGHT_PROG);

    return;

}

void mprog_give_trigger_inline(CHAR_DATA * mob, CHAR_DATA * ch, OBJ_DATA * obj)
{
...
void mprog_greet_trigger_inline(CHAR_DATA * ch)
{
    ...
    for (vmob = ch->in_room->people; vmob != NULL; vmob = vmob->next_in_room)
        if (IS_NPC(vmob)
            ...
            mprog_percent_check_inline(vmob, ch, NULL, NULL, GREET_PROG);
        else if (IS_NPC(vmob)
                 && (vmob->fighting == NULL)
                 && IS_AWAKE(vmob)
                 && (vmob->pIndexData->progtypes & ALL_GREET_PROG))
            mprog_percent_check_inline(vmob, ch, NULL, NULL, ALL_GREET_PROG);

    return;

}

void mprog_hitprcnt_trigger_inline(CHAR_DATA * mob, CHAR_DATA * ch)
{
...
void mprog_random_trigger_inline(CHAR_DATA * mob)
{

    if (mob->pIndexData->progtypes & RAND_PROG)
        mprog_percent_check_inline(mob, NULL, NULL, NULL, RAND_PROG);

    return;

}

void mprog_speech_trigger_inline(char *txt, CHAR_DATA * mob)
{
...
void mprog_bored_trigger(CHAR_DATA * mob)
{
    ...
    {
      mob->wait = 0;
      mprog_percent_check_inline(mob, NULL, NULL, NULL, BORED_PROG);
    }
...
bool mprog_exit_trigger_inline(CHAR_DATA *ch, int dir)
{
    CHAR_DATA *mob;
    MPROG_DATA *mprg;
